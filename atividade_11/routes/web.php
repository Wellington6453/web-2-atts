...
Route::middleware(['auth'])->group(function () {
    ...
    // Lista usuários com débito (apenas admin e bibliotecário)
    Route::get('/debits', [DebitController::class, 'index'])
        ->name('debits.index');

    // Quita débito de um usuário (apenas admin e bibliotecário)
    Route::patch('/debits/{user}/clear', [DebitController::class, 'clear'])
        ->name('debits.clear');
});
